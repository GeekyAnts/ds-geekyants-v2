...
Install per-component when you reach it (`pnpm add @radix-ui/react-<name>`), not all up front — this is a deliberately lean throwaway-friendly repo.
...
| Combobox / autocomplete | `Popover` + `cmdk` | `@radix-ui/react-popover` + `cmdk` | (ShadCN's Combobox recipe) filtered list, keyboard, `aria-activedescendant` |
